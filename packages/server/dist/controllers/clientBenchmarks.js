...
const createMyBenchmark = async (req, res) => {
    try {
        ...
        if (input.oldBenchmarkId) {
            ...
            const oldBenchmark = user.currentBenchmarks?.find((b) => b._id.toString() === input.oldBenchmarkId);
            if (!oldBenchmark) {
                res.status(404).json({
                    success: false,
        ...
        const createdBenchmark = userData.currentBenchmarks?.find((b) => b.id === newBenchmark._id.toString());
...
const updateMyBenchmark = async (req, res) => {
    try {
        ...
        let benchmarkLocation = null;
        const currentIndex = user.currentBenchmarks?.findIndex((b) => b._id.toString() === benchmarkId) ?? -1;
        if (currentIndex !== -1) {
            benchmarkLocation = 'current';
        }
        else {
            const historicalIndex = user.historicalBenchmarks?.findIndex((b) => b._id.toString() === benchmarkId) ?? -1;
        ...
        const arrayField = benchmarkLocation === 'current' ? 'currentBenchmarks' : 'historicalBenchmarks';
        const updateQuery = {};
        ...
        const benchmarks = benchmarkLocation === 'current'
 ...
 */
function getMeasurementTypeFromInput(input) {
    if (input.weightKg !== undefined)
        return 'weight';
    if (input.timeSeconds !== undefined)
        return 'time';
