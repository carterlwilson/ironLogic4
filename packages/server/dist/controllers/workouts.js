...
export const getCurrentWeekWorkouts = async (req, res) => {
    try {
        ...
        const activityTemplateIds = new Set();
        ...
        const templateMap = new Map(activityTemplates.map(template => [template._id.toString(), template]));
        ...
        const benchmarkTemplateNameMap = new Map(benchmarkTemplates.map((template) => [
            template._id.toString(),
            template.name
        ]));
        // 11. Create benchmark lookup map
        const benchmarkMap = new Map((user.currentBenchmarks || []).map(benchmark => [
        ...
        const enrichedDays = currentWeek.days.map(day => {
            const enrichedActivities = day.activities.map(activity => {
                ...
                let benchmarkWeightKg;
                let benchmarkName;
                let benchmarkTemplateId;
