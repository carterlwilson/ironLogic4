    ...
    try {
        ...
        const enrichedSchedules = schedules.map(schedule => {
            ...
            scheduleObj.coaches = scheduleObj.coachIds
                .map((coachId) => coachMap.get(coachId))
                .filter((coach) => coach !== undefined); // Filter out deleted coaches
            return scheduleObj;
        });
        const response = {
            success: true,
    ...
    try {
        ...
        const myTimeslots = schedules.map(schedule => {
            ...
            return {
                ...
                scheduleName: schedule.templateId?.name || 'Unknown',
                gymName: schedule.gymId?.name || 'Unknown',
                days: filteredDays,
            };
